...
<TABLE class=field>
  ...
  <TR>
    ...
    <TD><SELECT class=text name=typeId>
     <c:forEach items="${allType}" var="t"><OPTION value=${t.id}>${t.name}</OPTION></c:forEach>
    </SELECT></TD></TR>
  <TR>
...
name=floorage></TD></TR>
  ...
  <TR>
    ...
    <c:forEach items="${allDistrict}" var="d" > <OPTION value=${d.id}>${d.name}</OPTION></c:forEach>
  ...
  <TR>
    ...
    <TD><INPUT id="picture" class="text" type=file name="pfile"></TD>
  </TR>
  <TR>
